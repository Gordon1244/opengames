...
遊戲 ZIP 的根目錄必須包含 `index.html`。目前限制為壓縮檔 50 MiB、解壓後 250 MiB、最多 2,000 個檔案；禁止可執行檔、巢狀壓縮檔與路徑穿越。遊戲在獨立 iframe 沙箱執行，只能載入該作品自身與 `data:`／`blob:` 資源，並禁止表單、彈出視窗與頂層導覽。
