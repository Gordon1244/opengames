...
export default function LoginForm({ nextPath, locale, turnstileSiteKey }: { nextPath: string; locale: Locale; turnstileSiteKey: string }) {
  ...
  const captchaAction = mode === "register" ? "signup" : mode === "recover" ? "password_reset" : "login";
  ...
  async function signInWithPasskey() {
    ...
    if (!captchaToken) { setMessage(english ? "Complete the security check first." : "請先完成安全檢查。"); setBusy(false); return; }
    ...
    if (error) { setMessage(error.message); resetCaptcha(); setBusy(false); return; }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!captchaToken) { setMessage(english ? "Complete the security check first." : "請先完成安全檢查。"); setBusy(false); return; }
    if (mode === "recover") {
      const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: `${location.origin}/auth/callback?next=/account/password`, captchaToken });
    ...
    const result = mode === "login" ? await supabase.auth.signInWithPassword({ email, password, options: { captchaToken } }) : await supabase.auth.signUp({ email, password, options: { emailRedirectTo: `${location.origin}/auth/callback?next=${encodeURIComponent(nextPath)}`, captchaToken } });
    ...
    if (result.error || mode === "register") resetCaptcha();
    setBusy(false);
  }
