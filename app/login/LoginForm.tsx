...
export default function LoginForm({ nextPath, locale }: { nextPath: string; locale: Locale }) {
  ...
  return <div className="auth-card">
    <div className="auth-tabs"><button type="button" className={mode === "login" ? "active" : ""} onClick={() => setMode("login")}>{english ? "Sign in" : "登入"}</button><button type="button" className={mode === "register" ? "active" : ""} onClick={() => setMode("register")}>{english ? "Create account" : "建立帳號"}</button></div>
    ...
    <form onSubmit={submit} className="stack-form"><label>Email<input required type="email" name="email" autoComplete="email" /></label>{mode !== "recover" && <label>{english ? "Password" : "密碼"}<input required minLength={8} type="password" name="password" autoComplete={mode === "login" ? "current-password" : "new-password"} /></label>}<button className="form-submit" disabled={busy}>{busy ? (english ? "Working…" : "處理中…") : mode === "login" ? (english ? "Sign in to OpenGames" : "登入 OpenGames") : mode === "register" ? (english ? "Create a free account" : "免費建立帳號") : (english ? "Send reset link" : "寄送重設連結")}</button>{mode === "login" && <button className="text-button" type="button" onClick={() => { setMode("recover"); setMessage(""); }}>{english ? "Forgot password?" : "忘記密碼？"}</button>}{mode === "recover" && <button className="text-button" type="button" onClick={() => { setMode("login"); setMessage(""); }}>{english ? "Back to sign in" : "返回登入"}</button>}</form>
