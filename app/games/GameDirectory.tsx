...
export default function GameDirectory() {
  const [games, setGames] = useState<Game[]>([]); const [q, setQ] = useState(""); const [category, setCategory] = useState("全部"); const [loading, setLoading] = useState(true);
  useEffect(() => { fetch("/api/games").then((res) => res.json()).then((data) => setGames(data.games ?? [])).finally(() => setLoading(false)); }, []);
  ...
  return <>
    <div className="directory-tools"><label className="search-box"><span>⌕</span><input value={q} onChange={(event) => setQ(event.target.value)} placeholder="搜尋遊戲、創作者或標籤" /></label><div className="category-tabs">{categories.map((item) => <button className={item === category ? "active" : ""} key={item} onClick={() => setCategory(item)}>{item}</button>)}</div></div>
    ...
    {!loading && visible.length === 0 && <div className="empty-state"><strong>這裡暫時沒有符合的遊戲。</strong><span>換一個關鍵字，或成為第一位上傳這類作品的創作者。</span></div>}
  </>;
}
