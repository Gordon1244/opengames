...
export default function GameDirectory() {
  ...
  return <>
    ...
    <div className="directory-count">{loading ? "載入中…" : `${visible.length} 款可以立即遊玩的作品`}<span>依社群加權評價排序</span></div>
    <div className="game-grid directory-grid">{visible.map((game, index) => <a className="game-card" key={game.id} href={`/games/${game.slug}`}><GameVisual art={game.art} badge={game.badge} index={index + 1} /><div className="game-info"><div><h3>{game.title}</h3><p>by {game.creator}</p></div><span className="card-rating">★ {game.ratingCount ? game.ratingAverage.toFixed(1) : "新作"}<small>{game.ratingCount ? `${game.ratingCount} 則` : `${game.plays.toLocaleString()} 次遊玩`}</small></span></div></a>)}</div>
