...
export default function GameDirectory({ locale }: { locale: Locale }) {
  ...
  return <>
    ...
    <div className="game-grid directory-grid">{visible.map((game, index) => <a className="game-card" key={game.id} href={`/games/${game.slug}`}><GameVisual art={game.art} badge={game.badge} index={index + 1} /><div className="game-info"><div><h3>{game.title}</h3><p>by {game.creator}</p></div><span className="card-rating">★ {game.ratingCount ? game.ratingAverage.toFixed(1) : (game.isNew ? (english ? "NEW" : "新作") : (english ? "NOT RATED" : "尚無評價"))}<small>{game.ratingCount ? (english ? `${game.ratingCount} reviews` : `${game.ratingCount} 則`) : (english ? `${game.plays.toLocaleString("en-US")} plays` : `${game.plays.toLocaleString("zh-TW")} 次遊玩`)}</small></span></div></a>)}</div>
