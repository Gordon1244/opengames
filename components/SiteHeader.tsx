...
export async function SiteHeader() {
  ...
  return (
    <header className="site-header inner-header">
      ...
      <nav className="main-nav" aria-label={copy(locale, "主要導覽", "Main navigation")}><a href="/games">{copy(locale, "探索遊戲", "Explore")}</a><a href="/upload">{copy(locale, "上傳作品", "Upload")}</a><a href="/guidelines">{copy(locale, "社群規範", "Guidelines")}</a></nav>
      ...
      <details className="mobile-menu">
        ...
        <nav aria-label={copy(locale, "行動版導覽", "Mobile navigation")}><a href="/games">{copy(locale, "探索遊戲", "Explore")}</a><a href="/upload">{copy(locale, "上傳作品", "Upload")}</a><a href="/guidelines">{copy(locale, "社群規範", "Guidelines")}</a>{user ? <><a href={accountHref}>{accountLabel}</a><a href="/account/security">{copy(locale, "帳號安全", "Account security")}</a></> : <a href="/login">{copy(locale, "登入／註冊", "Sign in / Register")}</a>}<LanguageSwitch locale={locale} mobile /></nav>
      </details>
    </header>
  );
}
